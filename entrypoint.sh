...
if [[ -n $6 ]]; then
	cBitwidth=$(echo $6 | jq -r '."c-bitwidth" // empty')
	if [[ -n $cBitwidth ]]; then
		runCommand+=(--c-bitwidth $cBitwidth)
	fi
	fpRounding+=$(echo $6 | jq -r '."fp-rounding" // empty')
	if [[ -n $fpRounding ]]; then
		runCommand+=(--fp-rounding $fpRounding)
	fi
	useReal=$(echo $6 | jq -r '."use-real" // empty')
	if [ "XX $useReal" = 'XX "true"' ]; then
		runCommand+=(--use-real)
	fi
	zBitwidth=$(echo $6 | jq -r '."z-bitwidth" // empty')
	if [[ -n $zBitwidth ]]; then
...
if [[ -n $9 ]]; then
	parallelizationEnable=$(echo $9 | jq -r '."parallelization-enable" // empty')
	if [ "XX $parallelizationEnable" = "XX true" ]; then
		runCommand+=(--par)
		parallelizationCorePercentage=$(echo $9 | jq -r '."parallelization-core-percentage" // empty')
		if [[ -n $parallelizationCorePercentage ]]; then
			runCommand+=($parallelizationCorePercentage)
		fi
	fi
	parallelizationBranch=$(echo $9 | jq -r '."parallelization-branch" // empty')
	if [ "XX $parallelizationBranch" = "XX true" ]; then
		runCommand+=(--par-branch)
	fi
	parallelizationBranchReturn=$(echo $9 | jq -r '."parallelization-branch-return" // empty')
	if [ "XX $parallelizationBranchReturn" = "XX true" ]; then
		runCommand+=(--par-branch-return)
	fi
	parallelizationBranchPredNum=$(echo $9 | jq -r '."parallelization-branch-pred-num" // empty')
	if [[ -n $parallelizationBranchPredNum ]]; then
		runCommand+=(--par-branch-pred-num $parallelizationBranchPredNum)
	fi
	parallelizationBranchPredComplexity=$(echo $9 | jq -r '."parallelization-branch-pred-complexity" // empty')
	if [[ -n $parallelizationBranchPredComplexity ]]; then
		runCommand+=(--par-branch-pred-complexity $parallelizationBranchPredComplexity)
	fi
	parallelizationRewriting=$(echo $9 | jq -r '."parallelization-rw" // empty')
...
if [[ -n ${10} ]]; then
	dontSplitPfq=$(echo ${10} | jq -r '."dont-split-pfq" // empty')
	if [ "XX $dontSplitPfq" = "XX true" ]; then
		runCommand+=(--dont-split-pfq)
	fi
	splitAll=$(echo ${10} | jq -r '."split-all" // empty')
	if [ "XX $splitAll" = "XX true" ]; then
		runCommand+=(--split-all)
	fi
	splitContract=$(echo ${10} | jq -r '."split-contract" // empty')
	if [ "XX $splitContract" = "XX true" ]; then
		runCommand+=(--split-contract)
	fi
	splitIf=$(echo ${10} | jq -r '."split-if" // empty')
	if [ "XX $splitIf" = "XX true" ]; then
		runCommand+=(--split-if)
	fi
	splitMatch=$(echo ${10} | jq -r '."split-match" // empty')
...
# Rewriting options
if [[ -n ${11} ]]; then
	rwMax=$(echo ${11} | jq -r '."rewriting-max" // empty')
	if [[ -n $rwMax ]]; then
		runCommand+=(--rw-max $rwMax)
	fi
	rwTrace=$(echo ${11} | jq -r '."rewriting-trace" // empty')
	if [ "XX $rwTrace" = 'XX "true"' ]; then
		runCommand+=(--rw-trace)
	fi
	rwEvalTrace=$(echo ${11} | jq -r '."rewriting-eval-trace" // empty')
...
# SMT2 options
if [[ -n ${12} ]]; then
	elideEncoding=$(echo ${12} | jq -r '."elide-encoding" // empty')
	if [ "XX $elideEncoding" = "XX true" ]; then
		runCommand+=(--elide-encoding)
	fi
	rawInscription=$(echo ${12} | jq -r '."raw-inscription" // empty')
	if [ "XX $rawInscription" = "XX true" ]; then
		runCommand+=(--raw-inscription)
	fi
	rlimit=$(echo ${12} | jq -r '."rlimit" // empty')
	if [[ -n $rlimit ]]; then
		runCommand+=(--rlimit $rlimit)
	fi
	smt2Seq=$(echo ${12} | jq -r '."seq" // empty')
	if [ "XX $smt2Seq" = "XX true" ]; then
		runCommand+=(--smt2-seq)
	fi
	simplify=$(echo ${12} | jq -r '."simplify" // empty')
	if [ "XX $simplify" = "XX true" ]; then
		runCommand+=(--simplify)
	fi
	solverSat=$(echo ${12} | jq -r '."solver-sat" // empty')
	if [[ -n $solverSat ]]; then
		runCommand+=(--solver-sat $solverSat)
	fi
	solverValid=$(echo ${12} | jq -r '."solver-valid" // empty')
	if [[ -n $solverValid ]]; then
		runCommand+=(--solver-valid $solverValid)
	fi
	satTimeout=$(echo ${12} | jq -r '."sat-timeout" // empty')
	if [[ -n $satTimeout ]]; then
		runCommand+=(--sat-timeout $satTimeout)
	fi
	timeout=$(echo ${12} | jq -r '."timeout" // empty')
	if [[ -n $timeout ]]; then
		runCommand+=(--timeout $timeout)
	fi
	searchPC=$(echo ${12} | jq -r '."search-pc" // empty')
